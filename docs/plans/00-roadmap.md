...
|-------|------|--------|-----------|-----------|
| **P0** | 基础设施 | workspace | 依赖配置、测试框架、移除async-trait | `cargo build` 0 warnings，`cargo test` 全部通过 |
| **P1** | 核心原语完善 | axiom-core | Cell/Signal/Lens/Axiom/Witness 完整实现 | hello_cell 可以收发消息、产生Witness |
| **P2** | 事件存储 | axiom-store | Snapshot + Replay + EventStore完善 | 可以从EventLog重建任意状态 |
| **P3** | 运行时 | axiom-runtime | Mailbox + Bus + Dispatcher + Supervisor + Kernel | 多Cell可以通信、崩溃自动重启 |
...
| **P6** | 身份系统 | axiom-agent | Identity/Persona/PermissionSet | Agent Cell可以挂载身份、受权限约束 |
...
| **P9** | MCP桥接 | axiom-mcp | MCP Client/Server/Security | 可以连接MCP Server使用外部工具 |
| **P10** | LLM+工具 | axiom-llm, axiom-tool | LLM抽象/重试/缓存/结构化输出/Tool注册 | 可以调用LLM+工具完成简单任务 |
| **P11** | CLI基础 | axiom-cli | axm new/run/top/trace/why | 可以创建项目、启动系统、TUI监控 |
...
P4 监督层 (axiom-oversight) ──→ P5 可视化 (axiom-viz)
  ↓
P6 身份 → P7 技能 → P8 规则 (axiom-agent)
  ↓
P9 MCP (axiom-mcp)
  ↓
P10 LLM+工具 (axiom-llm, axiom-tool)
  ↓
P11 CLI基础 (axiom-cli)
  ↓
...
2. **测试通过**：`cargo test --workspace` 全部通过
3. **示例可运行**：如果是功能阶段，至少有一个 example 可运行
4. **无unsafe泄露**：unsafe代码全部在 `unsafe_impl` 模块中，且有SAFETY注释
5. **依赖方向正确**：`cargo tree` 验证无反向依赖
6. **文档完整**：所有public API有rustdoc注释
7. **可提交**：每个Phase作为一个或多个commit，commit message格式 `feat(scope): description`
