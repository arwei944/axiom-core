...
| P1 核心原语 | ✅ 完成 | Cell / Signal / Axiom / Witness / Entropy 核心抽象 (4/5 Lens待实现) | 197 |
...
## v0.2.0 开发计划 (见 [详细计划](plans/v0.2.0-development-plan.md))
...
1. **Lens 原语缺失** — 文档声称5个核心原语，但 Lens 只有 `LensId` 类型，无实际实现
2. **Witness 链无持久化** — Store 只有内存后端，重启后审计链丢失
3. **编译期→运行期约束断层** — 宏注入的约束在运行时 dispatch 路径上没有统一验证
