...
| 原语 | 状态 | 说明 |
|------|------|------|
| **Cell** | ✅ 完成 | 完整实现，支持监督策略、心跳检测、有界信箱 |
| **Signal** | ✅ 完成 | 类型化消息、VectorClock、Schema验证、序列化 |
| **Lens** | ⚠️ 待实现 | 仅定义了 `LensId` 类型，v0.2.0 完成 |
| **Axiom** | ✅ 完成 | 全局不变量约束、分布式注册表、链验证 |
| **Witness** | ✅ 完成 | 哈希链、自动记录、完整性验证，待持久化 |

### 16.2 自动注入机制

| 宏 | 状态 | 说明 |
|----|------|------|
| `#[cell(layer="...")]` | ✅ 完成 | 层标记、WitnessGenerator、监督策略 |
| `#[signal(kind="...", layer="...")]` | ✅ 完成 | 必需字段、Schema验证、序列化 |
| `#[tool]` | ✅ 完成 | Tool trait、权限检查、Witness记录 |
| `#[guard]` | ✅ 完成 | Guard trait、检查逻辑、Witness记录 |
| `#[capability(dim="...", version="...")]` | ✅ 完成 | 版本注册、兼容性策略、迁移链关联 |
...
| 优先级 | 任务 | 说明 |
|--------|------|------|
| P0 | Lens 原语实现 | 完成5原语故事 |
| P0 | Store 持久化 | SQLite + 文件系统后端 |
| P1 | 约束运行时统一 | 编译期约束在运行时总线层强制执行 |
| P1 | 现有 crate 深化 | identity/prompt/planner/memory |
| P2 | API 稳定 | v1 API 边界定义 |

详细计划请参考 [v0.2.0 开发计划](../plans/v0.2.0-development-plan.md)。
