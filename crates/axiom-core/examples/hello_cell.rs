...
impl Signal for HelloSignal {
    ...
    fn timestamp_ns(&self) -> u64 { axiom_core::signal::now_ns() }
    ...
    fn validate(&self) -> ValidationResult {
        ...
        result.merge(validators::require_max_length("message", &self.message, 1024));
        result
    }
...
async fn main() {
    ...
    for w in &witnesses {
        println!("  - {} [{:?}] (schema v{}, took {}us)",
            w.0.summary, w.0.outcome, w.0.schema_version.0, w.0.metrics.processing_time_us);
        println!("    version: crate={}, witness_schema=v{}",
            w.0.version_info.crate_version, w.0.version_info.witness_schema.0);
        println!("    payload: {} bytes, fingerprint: {:02x}{:02x}{:02x}...",
            w.0.payload_size_bytes, w.0.signal_fingerprint[0], w.0.signal_fingerprint[1], w.0.signal_fingerprint[2]);
    }
