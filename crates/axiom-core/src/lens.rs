...
pub trait Lens: Send + Sync {
    ...
    fn token_estimate(&self) -> usize { 0 }
}
...
impl<L1: Lens, L2: Lens> Lens for Lens2<L1, L2> {
    ...
    fn lens_id(&self) -> LensId { LensId::new("Lens2") }
    async fn project(&self) -> Result<Self::View> {
...
impl<L1: Lens, L2: Lens, L3: Lens> Lens for Lens3<L1, L2, L3> {
    ...
    fn lens_id(&self) -> LensId { LensId::new("Lens3") }
    async fn project(&self) -> Result<Self::View> {
...
impl<L: Lens> CachedLens<L> where L::View: Clone {
    pub fn new(inner: L) -> Self {
        Self { inner, cached_at: None, cached_value: None }
    }
...
impl<L: Lens + Clone> Lens for CachedLens<L> where L::View: Clone {
    type View = L::View;
    fn lens_id(&self) -> LensId { self.inner.lens_id() }
    async fn project(&self) -> Result<Self::View> { self.inner.project().await }
    async fn project_at(&self, clock: &VectorClock) -> Result<Self::View> {
