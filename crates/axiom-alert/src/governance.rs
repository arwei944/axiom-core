...
impl GovernanceMapper {
    pub fn map(
        alert: &Alert,
        _governor: &axiom_oversight::entropy_governor::EntropyGovernorCell,
    ) -> Option<axiom_oversight::entropy_governor::GovernanceAction> {
        match alert.status {
            AlertStatus::Firing => match alert.severity {
                ...
                crate::Severity::Warn => {
                    Some(axiom_oversight::entropy_governor::GovernanceAction::Warn {
            ...
            AlertStatus::Resolved => {
                Some(axiom_oversight::entropy_governor::GovernanceAction::None)
            }
