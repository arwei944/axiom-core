...
async fn test_restart_with_exponential_backoff() {
    ...
    supervisor.register_cell("test-cell", SupervisionStrategy::Restart { max_retries: 5 }).await;
...
async fn test_backoff_caps_at_30_seconds() {
    let supervisor = Supervisor::new();
    supervisor.register_cell("test-cell", SupervisionStrategy::Restart { max_retries: 10 }).await;
...
async fn test_circuit_breaker_stops_calls() {
    ...
    supervisor
        .register_cell(
            "test-cell",
            SupervisionStrategy::CircuitBreak { failure_threshold: 2, reset_after_ms: 100 },
        )
        .await;
...
async fn test_restart_count_tracking() {
    let supervisor = Supervisor::new();
    supervisor.register_cell("test-cell", SupervisionStrategy::Restart { max_retries: 3 }).await;
...
async fn test_circuit_break_decision_includes_until_time() {
    ...
    supervisor
        .register_cell(
            "test-cell",
            SupervisionStrategy::CircuitBreak { failure_threshold: 1, reset_after_ms: 5000 },
        )
        .await;
...
async fn test_stop_strategy_stops_after_first_panic() {
    let supervisor = Supervisor::new();
    supervisor.register_cell("test-cell", SupervisionStrategy::Stop).await;
...
async fn test_escalate_strategy() {
    let supervisor = Supervisor::new();
    supervisor.register_cell("test-cell", SupervisionStrategy::Escalate).await;
